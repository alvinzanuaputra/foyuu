...
const pengirim = "HEHEHEHE...";
const nomorWa = "6282334000058"; // awalan nomor 0 harus di awalin 62
const textWa = "Ah bisa aja kamu";
const pesan = `Pada akhirnya kita berdua menjadi dua orang penjahat: Kamu yang dengan hebatnya bisa mencuri hatiku, begitu juga aku yang berhasil mencuri hatimu DUarrrrrr mbeldoss`;

if (pengirim) {
  nama.innerHTML = pengirim;
  nama1.innerHTML = pengirim;
} else {
  nama.innerHTML = "Pengagum Kamu";
  nama1.innerHTML = "Pengagum Kamu";
}
...
hilang1.addEventListener('click', () => {
  ...
  Swal.fire({
    ...
    html: `<input type="text" id="login" class="swal2-input" placeholder="Nama Kamu Siapa ?">`,
    confirmButtonText: 'Kirim',
    showCancelButton: false,  // Tidak menampilkan tombol Cancel
    allowOutsideClick: false, // Menghilangkan kemampuan menutup pop-up dengan mengklik di luar pop-up
    focusConfirm: false,
    preConfirm: () => {
    ...
    let txt = `Hallo Hallo Haii Cantikk, nama kamu ${result.value.login}, nama kamu berawal dari huruf ${namaAwal.toUpperCase()}, ${pesan}`;
...
window.addEventListener('click', function(event) {
  if (hilang2.contains(event.target)) {
    hilang2.classList.add('none');
  }
});